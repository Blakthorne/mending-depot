...
export default function AddInventoryTransactionForm() {
    ...
    const { data: providersData, error: providersError } = useSWR('/api/providers')
    ...
    if (providersError) return <div>{ providersError }</div>
    ...
    const submitData = async e => {
        ...
        try {
            ...
            const body = { datePurchased, dateReceived, unitsPurchased, transactionCost, materialId, providerId }
    ...
    return (
        <div className="mt-16">
            <form
                autoComplete="off"
                onSubmit={submitData}>
                <FormTextInput onChange={(value) => setDatePurchased(value)} placeholder={ "'01 - 18 - 2017'" } input={ datePurchased } inputId={ "Date Purchased" } constraints={ ["date"] } errorMessage={ "Sorry, but that's not a real date." } dateIsValid={(validity) => setDatePurchasedValid(validity)}/>

                <FormTextInput onChange={(value) => setDateReceived(value)} placeholder={ "'02 - 09 - 2017'" } input={ dateReceived } inputId={ "Date Received" } constraints={ ["date"] } errorMessage={ "Sorry, but that's not a real date." } dateIsValid={(validity) => setDateReceivedValid(validity)}/>

                <FormTextInput onChange={(value) => setUnitsPurchased(value)} placeholder={ "'8'" } input={ unitsPurchased } inputId={ "Units Purchased" } constraints={ ["decimal"] } errorMessage={ "Please only enter a decimal value here." }/>

                <FormTextInput onChange={(value) => setTransactionCost(value)} placeholder={ "'43.99'" } input={ transactionCost } inputId={ "Transaction Cost" } constraints={ ["money"] } errorMessage={ "Please only enter a dollar value here." }/>
                ...
                <FormSelectInput onChange={(value) => setProviderId(value)} input={ providerId } inputId={ "Provider" } options={ providers } displayKey={ "providerName"} storeKey={ "id" }/>

                <FormSubmitButton requiredInputs={ [datePurchased, unitsPurchased, materialId, providerId] }/>
